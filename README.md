...
The routers can be migrated as a single component, but with prior migration of the subnet and network defined in the `external_gateway`, or with all of its compoenents by appending the `--include-dependencies` and  `--include-members` parameters. These will assure that all the components are migrated and attached to the specific router once migrated.
