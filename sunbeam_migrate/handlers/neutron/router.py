...
class RouterHandler(base.BaseMigrationHandler):
    ...
    def get_associated_resource_types(self) -> list[str]:
        """Routers depend on their external gateway network and subnets.

        The external subnets will be migrated as members of that network,
        but we list them as associated resources so their destination IDs
        are available when building external_gateway_info.
        """
        return ["network", "subnet"]

    def get_associated_resources(self, resource_id: str) -> list[tuple[str, str]]:
        """Return the external network and subnets this router depends on."""
        ...
        if network_id:
            ...
            # Include external gateway subnets as associated resources so their
            # destination IDs are available in migrated_associated_resources.
            # They'll be migrated as members of the external network, but listing
            # them here ensures we can look up their destination IDs without DB queries.
    ...
    def perform_individual_migration(
        ...
        if egi:
            ...
            if src_net_id:
                # Look up the migrated destination network ID from the manager-
                # provided associated resources.
                for (
                    assoc_type,
                    assoc_source_id,
                    assoc_dest_id,
                ) in migrated_associated_resources:
                    if assoc_type == "network" and assoc_source_id == src_net_id:
                        external_gateway_network_id = assoc_dest_id
                        break
            ...
            # External subnet(s) are listed as associated resources, so their
            # destination IDs are available in migrated_associated_resources.
            for fixed_ip in egi.get("external_fixed_ips", []) or []:
                ...
                dest_subnet_id: str | None = None
                for (
                    assoc_type,
                    assoc_source_id,
                    assoc_dest_id,
                ) in migrated_associated_resources:
                    if assoc_type == "subnet" and assoc_source_id == src_subnet_id:
                        dest_subnet_id = assoc_dest_id
                        break
    ...
    def connect_member_resources_to_parent(
        ...
        for resource_type, member_source_id, dest_subnet_id in migrated_member_resources:
